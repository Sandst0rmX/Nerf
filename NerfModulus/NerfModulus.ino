...
void loop() {
  ...
  if(revState){ //Checks to see if the rev trigger is held 
      ...
      if(fireMode == 2){//Checks if the blaster is currently in full auto mode
        if(triggerState){//Checks to see if the trigger is held
          //Fires a single shot but loops so fast that it is full auto
          fireBurst(1);
        }
      }
      else{
  ...
  if(analogRead(CYCLEMODE)>1000 && !hasCycled){//Checks to see if the touch sensor is pressed and the blaster has not changed fire mode yet
